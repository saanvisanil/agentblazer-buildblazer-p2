...
// ── In-memory cache ─────────────────────────────────────────────────────
// Public pages fetch the same handful of JSON files on every request.
// Without caching, each page load makes 2-4 round-trips to the GitHub API
// (~300-500 ms each), making the site feel slow. A short TTL (60 s) keeps
// content fresh enough — admin edits appear within a minute — while making
// repeat page loads near-instant.
const CACHE_TTL_MS = 60_000; // 60 seconds
const cache = new Map(); // path → { data, sha, expiresAt }
...
function setCache(path, data, sha) {
  cache.set(path, { data, sha, expiresAt: Date.now() + CACHE_TTL_MS });
}

// Called after a write so the next read returns fresh content immediately
// instead of serving stale data for up to CACHE_TTL_MS.
...
export async function readJsonFile(path, { skipCache = false } = {}) {
  // Serve from cache if available and not explicitly bypassed
  if (!skipCache) {
    const hit = getCached(path);
    if (hit) return { data: hit.data, sha: hit.sha };
  }
...
export async function writeJsonFile(path, data, sha, message) {
  ...
  // Update cache immediately so the admin sees their own changes right away
  setCache(path, data, json.content.sha);
